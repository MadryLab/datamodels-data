"""
    A custom version of FMoW, adapted from WILDS.
"""
...
class FMoWDataset(WILDSDataset):
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official', seed=111, use_ood_val=True):
        ...
        if self._split_scheme.startswith('time_after'):
            ...
            # Restrict training set to single year (2012 by default).
            train_year_start = datetime.datetime(year-4, 1, 1, tzinfo=pytz.UTC)
            self.id_mask = np.asarray(pd.to_datetime(self.metadata['timestamp']) >= train_year_start) & ~self.ood_mask
        else:
        ...
        for split in self._split_dict.keys():
            ...
            if split == 'test':
                ...
                test_mask = np.asarray(self.metadata['split'] == 'test')
                idxs = idxs[self.id_mask & test_mask]
            elif split == 'id_val':
                val_mask = np.asarray(self.metadata['split'] == 'val')
                idxs = idxs[self.id_mask & val_mask]
            else:
                split_mask = np.asarray(self.metadata['split'] == split)
                idxs = idxs[self.id_mask & split_mask]
        ...
        #self._y_array = np.ones_like(self._y_array)
...
if __name__ == '__main__':
    ...
    print(f'> num_train: {len(ds_train)} | num_test: {len(ds_test)}')